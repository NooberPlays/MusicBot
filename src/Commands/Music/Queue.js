...
module.exports = class QueueCommand extends Command {
  ...
  async exec(msg) {
    try {
      ...
      const pages = chunk(GuildPlayers?.queue.map((x, i) => `\`${i + 1}\` ${x.title} [${x.requester}]`), 10);
      const embed = CreateEmbed('info').setAuthor(`${msg.guild?.name} queue list. Page ${index + 1} of ${pages.length}`, msg.guild.iconURL());
